...
find . ! -path './.github/**' ! -name '.pre-commit-config.yaml' ! -name 'kustomizeconfig.yaml' ! -name '*-values.yaml' ! -name 'values.yaml' -type f -name '*.yaml' -print0 | while IFS= read -r -d $'\0' file;
  do
